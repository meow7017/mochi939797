...
// 调度与单聊一致：全局唯一闹钟、每次排期先清旧、开关关闭时短轮询等恢复、
// 只启动一次、回前台统一补发——避免“发一两次就再也不触发”。
let gcAsTimer = null;    // 全局唯一排期闹钟
let gcAsBooted = false;  // 是否已启动（防 restore 与 3s 兜底双触发叠加成两条链）
function gcAsCfg() {
  ...
  return {
    en: get('en', '1'),
    prob: Number(get('prob', 25)),
    min: Number(get('min', 20)),
    max: Number(get('max', 60)),
    countMin: Number(get('count-min', 1)),
...
// 唯一调度入口：先清旧闹钟再排下一个，保证全项目只有一条链、永不叠加
function gcScheduleAutoSend() {
  clearTimeout(gcAsTimer);
  const c = gcAsCfg();
  // 总开关关闭：用短轮询反复检查，用户一旦打开 30 秒内自动恢复（等同单聊）
  if (c.en !== '1') {
    gcAsTimer = setTimeout(gcScheduleAutoSend, 30000);
    return;
  }
  const min = Math.min(600, Math.max(1, c.min || 20));
  const max = Math.max(min, Math.min(600, c.max || 60));
  const delay = (min + Math.random() * Math.max(1, max - min)) * 60000;
  gcAsTimer = setTimeout(function () {
    gcTryAutoSend();            // 本次没命中也没关系
    gcScheduleAutoSend();       // 无论如何都排下一轮 → 永不间断
  }, delay);
}
function gcBootAutoSend() {
  if (gcAsBooted) return;       // 只真正启动一次
  gcAsBooted = true;
  gcScheduleAutoSend();
}
...
function _gcOnFg() {
  ...
  _gcFgAt = now;
  gcTryAutoSend();
  gcScheduleAutoSend(); // 统一走唯一调度入口（内部先清旧再排），不会叠加
}
